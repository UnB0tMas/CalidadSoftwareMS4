...
@Service
public class InventarioEventoPendienteServiceImpl
        implements InventarioEventoPendienteService {

    private static final Logger log =
            LoggerFactory.getLogger(
                    InventarioEventoPendienteServiceImpl.class
            );

    private static final String RECURSO_EVENTO_PENDIENTE =
            "Evento pendiente de inventario";

    private static final String ENTIDAD_EVENTO_PENDIENTE =
            "INVENTARIO_EVENTO_PENDIENTE_MS4";

    private static final int MAX_RECONCILIACION_BATCH =
            500;
    ...
    public InventarioEventoPendienteServiceImpl(
            InventarioEventoPendienteMs4Repository repository,
        ...
        this.auditoriaFuncionalService =
    ...
    @Override
    @Transactional
    public InventarioEventoPendienteResponseDto registrarPendiente(
            Venta venta,
            VentaDetalle detalle,
            TipoComandoStock tipoComando,
            String payloadJson
    ) {
        validator.validarRegistroPendiente(
                venta,
                detalle,
                tipoComando,
                payloadJson
        ...
        InventarioEventoPendienteMs4 existente =
                repository
        ...
        if (existente != null) {
            ...
            throw new ConflictException(
                    "Ya existe un evento pendiente inactivo con la misma clave de idempotencia."
            );
        }
        ...
        InventarioEventoPendienteMs4 entity =
                InventarioEventoPendienteMs4.builder()
                        .idVenta(
                                venta.getId()
                        )
                        .codigoVenta(
    ...
    public PageResponseDto<InventarioEventoPendienteResponseDto>
    listar(
            InventarioEventoPendienteFilterDto filter,
            PageRequestDto page
    ) {
        ...
        contingenciaPolicy
                .authorizeConsultarEventosPendientes(
                        actor
                );

        validator.validarFiltro(filter);

        Page<InventarioEventoPendienteMs4> result =
                repository.findAll(
                        InventarioEventoPendienteSpecification
                                .build(filter),
    ...
    @Transactional(readOnly = true)
    public PageResponseDto<InventarioEventoPendienteResponseDto>
    listarPendientesParaMs3(
            InventarioEventoPendienteFilterDto filter,
            PageRequestDto page
    ) {
        validator.validarFiltro(filter);

        Specification<InventarioEventoPendienteMs4>
                specification =
                Specification
                        .where(
                                InventarioEventoPendienteSpecification
    ...
    @Override
    @Transactional
    public InventarioEventoPendienteResponseDto
    marcarSincronizado(
            Long idPendiente,
            String detalleResultado
    ) {
        InventarioEventoPendienteMs4 evento =
                resolverActivo(idPendiente);

        validator.validarMarcarSincronizado(
                evento
        );

        evento.setEstadoSincronizacion(
                EstadoSincronizacionInventario.SINCRONIZADO
        );

        evento.setFechaSincronizacion(
                LocalDateTime.now(clock)
        );
    ...
    @Override
    @Transactional
    public InventarioEventoPendienteResponseDto marcarError(
            Long idPendiente,
            String errorDetalle
    ) {
        InventarioEventoPendienteMs4 evento =
        ...
        evento.setFechaUltimoReintento(
                LocalDateTime.now(clock)
        );
    ...
    registrarResultadoSincronizacion(
            StockSyncResultRequestDto request
    ) {
        validarResultadoSincronizacion(
                request
        );

        if (
                Boolean.TRUE.equals(
                        request.sincronizado()
                )
        ) {
            return marcarSincronizado(
                    request.idEventoPendiente(),
                    request.detalleResultado()
            );
        }
    ...
    @Override
    @Transactional
    public InventarioEventoPendienteResponseDto reintentar(
            Long idPendiente,
            AuthenticatedUserContext actor
    ) {
        try {
            contingenciaPolicy.authorizeReconciliar(
                    actor
            );
            ...
            evento.setCantidadReintentos(
                    nextRetry(
                            evento.getCantidadReintentos()
                    )
            );
            ...
            log.error(
                    "Error técnico reintentando evento de inventario. idPendiente={}, actorIdUsuarioMs1={}",
                    idPendiente,
                    actor == null
                            ? null
                            : actor.idUsuarioMs1(),
            ...
            throw internalError(
                    "No se pudo reintentar el evento pendiente de inventario.",
                    ex
            );
        }
    }
    ...
    @Override
    @Transactional
    public ContingenciaReconciliacionResponseDto
    reconciliarPendientes(
        ...
        try {
            ...
            long totalReintentables =
                    contarPendientesActivos();

            List<InventarioEventoPendienteMs4> eventos =
                    repository.findPendientesForUpdate(
                            estadosReintentables(),
                            PageRequest.of(
                                    0,
                                    MAX_RECONCILIACION_BATCH
                            )
                    );
            ...
            log.error(
                    "Error técnico reconciliando eventos pendientes de inventario. actorIdUsuarioMs1={}",
                    actor == null
                            ? null
                            : actor.idUsuarioMs1(),
                    ex
            ...
            throw internalError(
                    "No se pudo reconciliar los eventos pendientes de inventario.",
                    ex
            );
        }
    }
    ...
    public long contarPendientesActivos() {
        return repository
                .countByEstadoSincronizacionInAndEstadoTrue(
                        estadosReintentables()
                );
    }

    private void validarResultadoSincronizacion(
            StockSyncResultRequestDto request
    ) {
        if (request == null) {
            throw new ValidationException(
                    "El resultado de sincronización es obligatorio."
            );
        }

        if (
                request.idEventoPendiente() == null
                        || request.idEventoPendiente() <= 0
        ) {
    ...
    private InventarioEventoPendienteMs4 resolverActivo(
            Long idPendiente
    ) {
        if (
                idPendiente == null
            ...
            throw NotFoundException.byId(
                    RECURSO_EVENTO_PENDIENTE,
                    idPendiente
            );
        }

        return repository
                .findById(idPendiente)
                .filter(
                        InventarioEventoPendienteMs4::isActivo
                )
                .orElseThrow(
                        () ->
    ...
    private List<EstadoSincronizacionInventario>
    estadosReintentables() {
        return List.of(
                EstadoSincronizacionInventario.PENDIENTE,
                EstadoSincronizacionInventario.ERROR,
    ...
    private String resolveTopicDestino(
            TipoComandoStock tipoComando
    ) {
        if (
                tipoComando
                        == TipoComandoStock.RECONCILIAR_STOCK
        ) {
            return kafkaTopicProperties
                    .stockReconciliationTopic();
        }
    ...
    private Map<String, Object> detalleAuditoria(
        ...
        detalle.put(
                "codigoVenta",
                evento.getCodigoVenta()
        );

        detalle.put(
                "idVentaDetalle",
                evento.getIdVentaDetalle()
        );
        ...
        detalle.put(
                "estadoSincronizacion",
                evento.getEstadoSincronizacion()
        );
        ...
        if (
                detalleResultado != null
                        && !detalleResultado.isBlank()
        ) {
            detalle.put(
                    "detalle",
                    detalleResultado.trim()
            );
        }
