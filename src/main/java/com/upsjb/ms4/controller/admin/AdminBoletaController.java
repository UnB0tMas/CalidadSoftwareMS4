...
@RestController
@Validated
@RequestMapping(ApiPaths.ADMIN_BOLETAS)
@Tag(name = "Admin - Boletas", description = "Consulta, preview HTML, PDF en vivo y reenvío administrativo de boletas.")
...
public class AdminBoletaController {
    ...
    @GetMapping
    @Operation(summary = "Listar boletas administrativamente")
    public ResponseEntity<ApiResponseDto<PageResponseDto<BoletaResponseDto>>> listarBoletas(
        ...
        PageResponseDto<BoletaResponseDto> data = boletaService.listarAdmin(filter, page, actor);

        return ResponseEntity.ok(responseFactory.ok(
                data,
                "Boletas consultadas correctamente.",
                servletRequest
        ));
    }

    @GetMapping("/{idBoleta}")
    @Operation(summary = "Obtener detalle administrativo de boleta")
    public ResponseEntity<ApiResponseDto<BoletaDetailResponseDto>> obtenerBoleta(
            @PathVariable @Positive(message = "El idBoleta debe ser positivo.") Long idBoleta,
            HttpServletRequest servletRequest
    ) {
        AuthenticatedUserContext actor = authenticatedUserResolver.current();
        BoletaDetailResponseDto data = boletaService.obtenerDetalleAdmin(idBoleta, actor);
    ...
    @Operation(summary = "Renderizar preview HTML administrativo de boleta")
    ...
    @Operation(summary = "Generar PDF administrativo en vivo de boleta")
    ...
    @Operation(summary = "Programar reenvío administrativo de boleta por correo")
    public ResponseEntity<ApiResponseDto<CorreoOutboxResponseDto>> reenviarBoleta(
        ...
        CorreoOutboxResponseDto data = boletaService.programarCorreoBoleta(
                idBoleta,
