...
public class KafkaProbeFromMs2Consumer {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(
                    KafkaProbeFromMs2Consumer.class
            );
    ...
    @KafkaListener(
            ...
            groupId = "${app.kafka.probe.consumer-group:ms4-probe-consumer}-from-ms2",
            clientIdPrefix = "ms4-probe-from-ms2",
    ...
    public void consumeProbeFromMs2(
        ...
        if (!hasPayload(record)) {
            log.warn(
                    "[KAFKA-PROBE][MS4] Probe vacío recibido desde MS2. topic={}, partition={}, offset={}, key={}",
                    record == null
                            ? null
                            : record.topic(),
                    record == null
        ...
        try {
            ...
            log.error(
                    "[KAFKA-PROBE][MS4] No se pudo enviar el ACK hacia MS2. probeId={}, topic={}, key={}",
                    probe.probeId(),
                    ackTopic,
                    ackKey,
                    ex
    ...
    private KafkaProbePayload deserialize(
        ...
        try {
            ...
            log.error(
                    "[KAFKA-PROBE][MS4] Probe con JSON inválido recibido desde MS2. topic={}, partition={}, offset={}, key={}, payload={}",
                    record.topic(),
                    record.partition(),
                    record.offset(),
                    record.key(),
