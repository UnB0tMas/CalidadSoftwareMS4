...
public class KafkaProbeFromMs3Consumer {
    ...
    private static final Logger log = LoggerFactory.getLogger(KafkaProbeFromMs3Consumer.class);
    ...
    public void consumeProbeFromMs3(
        ...
        try {
            if (record == null || record.value() == null || record.value().isBlank()) {
                log.warn("[KAFKA-PROBE][MS4] Probe vacío recibido desde MS3.");
            ...
            KafkaProbeAckPayload ack = KafkaProbeAckPayload.ackToMs3(
                    probe,
            ...
            String ackTopic = properties.ms4ToMs3AckTopic();
            ...
            log.error(
                    "[KAFKA-PROBE][MS4] Error consumiendo probe desde MS3. topic={}, partition={}, offset={}, key={}",
